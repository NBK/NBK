...
bool RedirectIAT( SDLLHook* DLLHook, PIMAGE_IMPORT_DESCRIPTOR pImportDesc, PVOID pBaseLoadAddr )
{
    ...
    while ( pIteratingIAT->u1.Function )
    {
        ...
        if ( !IMAGE_SNAP_BY_ORDINAL( pINT->u1.Ordinal ) )  // import by name
        {
            ...
            while ( FHook->Name )
            {		
				//MessageBox(NULL,(char*)pImportName->Name,"",MB_OK);
...
bool HookAPICalls( SDLLHook* Hook, HMODULE *lpMod )
{
    if ( !Hook )
        return false;

	HMODULE hModEXE = lpMod?*lpMod:GetModuleHandle(0);
    ...
    while ( pImportDesc->FirstThunk )
    {
		...
		//MessageBox(NULL,pszImportModuleName,"",MB_OK);
        ...
        if ( lstrcmpi( pszImportModuleName, Hook->Name ) == 0 )
        {
			...
			/*char ch[32];
			MessageBox(NULL,pszImportModuleName,"",MB_OK);
			MessageBox(NULL,itoa(pImportDesc->OriginalFirstThunk,ch,10),"",MB_OK);
			MessageBox(NULL,itoa(pImportDesc->FirstThunk,ch,10),"",MB_OK);*/
...
static PIMAGE_IMPORT_DESCRIPTOR GetNamedImportDescriptor(HMODULE hModule, LPCSTR szImportModule)
{
	...
	if (IsBadReadPtr(pDOSHeader, sizeof(IMAGE_DOS_HEADER)) ||
		...
		PIMAGE_IMPORT_DESCRIPTOR pImportDesc =
			MakePtr(PIMAGE_IMPORT_DESCRIPTOR, pDOSHeader,
			pNTHeader->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress);
