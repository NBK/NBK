...
MilkshapeModel::MilkshapeModel(bool transparent, bool animated, bool repeat_animation)
{
	this->transparent=transparent;
	this->animated=animated;
	m_looping=repeat_animation;
}
...
//                cout << "Couldn't open the model file." << endl;
	...
	char pathTemp[256+1];
	int pathLength;
	for ( pathLength = (int)strlen( filename ); --pathLength; ) {
	...
	if (animated)
	{
		...
		for ( i = 0; i < m_numJoints; i++ )
		{
			...
			if ( strlen( pJoint->m_parentName ) > 0 )
			{
				...
				{
					if ( strcmp( pNameList[j].m_pName, pJoint->m_parentName ) == 0 )
					{
