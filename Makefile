...
CDIR=$(MOUNT_DIR)/src/Game
CMLDIR=$(MOUNT_DIR)/dep
SDLHDIR=$(MOUNT_DIR)/dep/SDL12
GLEWDIR=$(MOUNT_DIR)/dep/glew
...
ifeq ($(shell pkg-config --exists lua && echo yes),yes)
...
ifneq (,$(findstring "$(PLATFORM)", "linux" "gnu_kfreebsd" "kfreebsd-gnu"))
  ...
  NBKE_LIBS += $(LUA_LIBS)
  NBKE_CXXFLAGS += -I$(GLEWDIR)/include

  ifeq ($(USE_LOCAL_HEADERS),1)
    NBKE_CXXFLAGS += -I$(SDLHDIR)/include -DUSE_LOCAL_HEADERS
    NBKE_CXXFLAGS += -I$(MOUNT_DIR)/dep
  else
